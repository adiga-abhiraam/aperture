...
export const ATP_META = {
  ...
  environment: [
    ...
    ['Models', 'Local Multimodal Encoders'],
...
export const ACCEPTANCE_CASES: AcceptanceCase[] = [
  ...
  },
  {
    ...
    title: 'An alternative model answers the same query',
    precondition: 'API key configured.',
    steps: ['Select alternative provider', 'Run a query', 'Inspect provider and frames_sampled'],
    expected: 'Sections returned; response discloses frame sampling.',
    actual: 'Alternative provider, frames_sampled=16, 4 sections with correct timestamps.',
    status: 'pass',
    automated: false,
  },
  {
  ...
  },
  {
    ...
    actual: 'Blocked: Drive access not enabled on the project.',
    status: 'blocked',
    automated: false,
  },
  {
...
export const DESIGN_SECTIONS: AtpSection[] = [
  ...
  },
  {
    ...
    points: [
      ...
      'Local vector store: 512/512/1024/1024 across all four modalities.',
    ],
  },
  {
    id: 'normalisation',
  ...
  },
  {
    ...
    points: [
      'multimodal_runtime: upload, ACTIVE wait, structured JSON, retry classification',
      'fallback_runtime: frame sampling, strict json_schema',
  ...
  },
  {
    id: 'scalability',
    title: 'Scalability',
    body: 'The single-call paths need no persistent service beyond the engine process, while library search scales horizontally behind Qdrant.',
    points: [
      'Stateless quick_search and quick_index: zero database dependencies',
      'Optional local Qdrant container: single command startup',
      'Local Qdrant vector database: high-performance similarity search',
    ],
  },
  {
    id: 'security',
    title: 'Security and privacy',
    body: 'Zero-retention defaults: uploads and cut clips live in temporary workspaces removed after processing. All operations run locally.',
    points: [
      'Zero external data transmission',
      'Automatic scrubbing of sensitive patterns in all diagnostics',
...
export const DEPENDENCIES = [
  { name: 'FastAPI + Uvicorn', role: 'Local HTTP service', licence: 'MIT / BSD', cost: 'Free', risk: 'Low, widely adopted' },
  { name: 'Local Neural Pipeline', role: 'Multimodal processing', licence: 'Apache 2.0', cost: 'Free local', risk: 'Hardware resource usage' },
  { name: 'ffmpeg / ffprobe', role: 'Probe, cut, transcode, sample', licence: 'LGPL / GPL', cost: 'Free', risk: 'Must exist on host' },
  ...
  { name: 'Qdrant', role: 'Vector store', licence: 'Apache 2.0', cost: 'Free local self-hosted', risk: 'Optional; the single-call path does not use it' },
  { name: 'Neural Feature Encoders', role: 'On-device feature extraction', licence: 'Apache 2.0 / MIT', cost: 'Free, runs locally', risk: 'Initial asset setup' },
  { name: 'Local Vector Store', role: 'Embedded vector database', licence: 'Apache 2.0', cost: 'Free', risk: 'None' },
]
