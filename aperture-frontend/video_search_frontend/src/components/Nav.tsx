          ...
          <div className="hidden flex-1 items-center justify-end gap-5 md:flex lg:gap-7">
            {LINKS.map((link) => (
              <a
                ...
                className={`whitespace-nowrap text-sm font-medium transition-colors hover:text-white ${pathname === link.path ? 'text-white' : 'text-white/70'
                  }`}
              >
                {t[link.key]}
              </a>
        ...
        <div className="mt-3 grid grid-cols-3 gap-1 border-t border-white/10 pt-3 md:hidden">
          {LINKS.filter((link) => ['/', '/how-it-works', '/preprocess'].includes(link.path)).map((link) => (
            <a
              ...
              href={link.path}
              onClick={go(link.path)}
              className={`rounded-full px-2 py-2 text-center text-xs font-medium transition-colors hover:bg-white/5 hover:text-white ${pathname === link.path ? 'bg-white/5 text-white' : 'text-white/60'
                }`}
            >
              {t[link.key]}
            </a>
